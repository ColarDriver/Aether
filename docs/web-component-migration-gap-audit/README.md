...
- `UserMessageBlock.tsx`, `AssistantMessageBlock.tsx`, `ThinkingBlock.tsx`, `StreamingStatusBlock.tsx`, and `SystemNoticeBlock.tsx` cover the base message states.
...
- `ToolCallBlock.tsx`, `ToolCallGroup.tsx`, `ToolResultBlock.tsx`, `TerminalChrome.tsx`, `ToolResultPreview.tsx`, `TodoListPreview.tsx`, and `InlineTaskSummary.tsx` cover shell chrome, todo previews, file-read previews, file-change previews, notebook edit previews, grep/search previews, LSP previews, browser-result previews, web-search previews, image artifacts, non-image artifact bundles, spill notices, and first-class task/subagent summaries.
...
- `PermissionDialog.tsx`, `ApprovalDialog.tsx`, `PromptContent.tsx`, `PermissionRequestBlock.tsx`, `ApprovalRequestBlock.tsx`, and `AskUserQuestionBlock.tsx` cover prompt modal and historical prompt rendering.
...
| File/search/web/tool previews | Partial, improved | `ToolResultPreview` | File edit, notebook edit, LSP, browser, file-read, search, web, task, todo, image artifacts, non-image artifact bundles, and spill notices now have first-pass previews. Remaining gaps are browser visual acceptance, hosted web-search variants, notebook rich diffs, and deeper arbitrary artifact bundle actions. |
| Todo preview | Mostly complete | `TodoListPreview` | Needs parity validation against live tool payloads. |
| Subagent/task inline summary | Partial, improved | `InlineTaskSummary`, `TaskNotificationBlock`, `ToolResultPreview` | Task drawer/A2A detail links and nested child-task summaries still need deeper backend integration. |
...
| Ask user question | Foundation complete | `AskUserQuestionBlock`, `PromptContent` | Needs more answer-shape coverage and live provider acceptance. |
...
| Message action bar | Minimal | `MessageActionBar` | Copy exists; retry/edit/quote/rewind/fork/checkpoint are missing. |
...
1. Message lifecycle actions: retry assistant response, edit user prompt, quote, fork/rewind/checkpoint. These require backend contracts before UI should pretend they work.
...
4. Deep task/subagent UI: inline summaries now exist, but task detail drawer and A2A/nested child task rendering need to be connected to live task services.
...
6. Tool-specific preview breadth: shell/read/search/web/todo/task/images plus file edit, notebook edit, LSP, browser-result, diagnostics, artifact bundle, and spill-notice previews are started; browser visual acceptance, hosted web-search variants, notebook rich diffs, and deeper artifact actions still need dedicated renderers.
...
Continue beyond the first-pass file edit/write, notebook edit, LSP, browser/screenshot, web, image, diagnostics, artifact, spill-notice, and task previews. Add notebook rich diffs, browser visual artifact routing, hosted web-search variants, web fetch document cards, and deeper artifact bundle actions. Keep generic JSON/code fallback for unknown tools.
