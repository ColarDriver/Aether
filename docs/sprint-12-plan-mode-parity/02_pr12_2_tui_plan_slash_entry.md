...
如果已经是 plan mode 但输入 `/plan <description>`，仍应把 description 继续提交为 query。不要因为已经处于 plan mode 就吞掉用户的新规划请求。
...
1. 调用 `plan.current`。
2. 如果 `has_plan=false` 或 `plan_path=null`，输出：

```text
No plan written yet.
```

3. 如果有 `plan_path`，调用本地 editor 打开。
4. 如果 editor 环境不可用，输出清晰提示，例如：

```text
Could not open plan: EDITOR is not configured.
```

5. 如果 editor 进程启动失败，显示失败原因，但不要退出 TUI。
