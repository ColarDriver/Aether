...
from aether.agents.runtime.turn_runner import (
...
class AgentEngine:
    ...
    def run_loop(self, request: EngineRequest) -> EngineResult:
        ...
        try:
            return self._turn_runner.run(TurnRunInput(request=request)).result
        finally:
            self._current_session_id = None

    @staticmethod
    def _is_continue_loop_finalization(finalized: Any) -> bool:
        return getattr(finalized, "final_response", None) is _CONTINUE_LOOP_SENTINEL
