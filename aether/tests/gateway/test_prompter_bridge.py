...
class GatewayPrompterBridgeTests(unittest.TestCase):
    ...
    def test_ask_questions_maps_schema_and_returns_answers(self) -> None:
        ...
        questions = [
            {
                ...
                "options": [
                    {"id": "a", "label": "API", "description": "HTTP service"},
                    {"id": "b", "label": "CLI", "description": "Terminal tool"},
                ],
            },
        ...
        self.assertEqual(params["questions"][0]["header"], "Target")
        self.assertEqual(
            params["questions"][0]["options"],
            [
                {"label": "API", "description": "HTTP service"},
                {"label": "CLI", "description": "Terminal tool"},
            ],
        )
        self.assertEqual(params["questions"][1]["kind"], "open")

    def test_ask_questions_decodes_user_action(self) -> None:
        prompter = GatewayPrompter(session_id="ses_1", run_id="run_1")
        questions = [
            {
                "id": "q1",
        ...
        with mock.patch(
            "aether.gateway.handlers.prompter_bridge.reverse_rpc.call",
            return_value={"kind": "questions", "answers": {}, "action": "skip"},
        ):
            result = prompter.ask_questions(questions, timeout=1.0)
        self.assertEqual(result, {"__user_action": "skip"})
